...
from src.rest_api.pydantic_data_types import UserInDB, StationCondition, StationConditionResponse
...
@router.post(path="/conditions", status_code=201)
async def insert_weather_conditions_data(
...
) -> StationConditionResponse:
    if not auth_station:
        raise HTTPException(
            status_code=401,
            detail="Unauthorized access - request denied.",
    ...
    return StationConditionResponse(station_id=auth_station.station_id, **station_condition.model_dump())